        ...
        {CHARACTERS.map(c => (
          <div key={c.id} onClick={() => setSelectedChar(c)} style={charTabStyle(selectedChar.id === c.id)}>
            <div style={iconWrapperStyle(selectedChar.id === c.id)}>
              <img src={`/${c.id}.png`} alt="" style={iconImgStyle} onError={(e) => { e.target.style.display = 'none'; e.target.parentElement.innerHTML = `<div style="width:100%;height:100%;display:flex;align-items:center;justify-content:center;background:#222;color:#0ff;">${c.name[0]}</div>`; }} />
            </div>
          </div>
        ))}
      </div>
...
const charBgOverlay = { position: 'absolute', top: '50%', left: '50%', transform: 'translate(-50%, -50%)', width: '100%', height: '100%', backgroundSize: 'contain', backgroundPosition: 'center', backgroundRepeat: 'no-repeat', opacity: 0.2, zIndex: 0, pointerEvents: 'none' };
...
const myCharSelect = { background: '#000', color: '#0ff', border: '1px solid #0ff', borderRadius: '4px' };
const vsLabel = { color: '#f44', fontWeight: 'bold' };
const charNavContainer = { display: 'flex', overflowX: 'auto', padding: '10px', gap: '10px', background: '#000', zIndex: 10 };
const charTabStyle = (active) => ({ opacity: active ? 1 : 0.3, cursor: 'pointer' });
...
const statusRowStyle = { display: 'flex', gap: '10px', background: '#111', padding: '10px', borderRadius: '8px' };
...
const saveBtn = { background: '#0ff', border: 'none', borderRadius: '3px', fontSize: '10px' };
const graphContainer = { height: '30px' };
const linkGridStyle = { display: 'flex', alignItems: 'center' };
const linkBtnStyle = (color) => ({ color, border: `1px solid ${color}`, padding: '5px', borderRadius: '4px', fontSize: '10px', textDecoration: 'none' });
...
const mainArea = { minHeight: '200px', background: '#000', color: '#eee', padding: '10px', borderRadius: '8px', border: '1px solid #333' };
const comboSection = { display: 'flex', flexDirection: 'column', gap: '10px' };
const comboCard = { background: '#111', padding: '10px', borderRadius: '8px', border: '1px solid #333', display: 'flex', flexDirection: 'column', gap: '8px' };
...
const alertBox = (color) => ({ border: `1px solid ${color}`, padding: '10px', borderRadius: '8px' });
